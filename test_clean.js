...
<head>
    ...
    <title>CERN BL4S — Modular Event Explorer & Beamline DAQ</title>
    <!-- Socket.IO & Apache ECharts -->
    <script src="https://cdn.socket.io/4.7.5/socket.io.min.js">

    <script src="https://cdn.jsdelivr.net/npm/echarts@5.5.0/dist/echarts.min.js">

    <!-- Three.js & OrbitControls for 3D Event Display -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.js">

    <!-- Post-processing (bloom glow) & real-time floor reflections for photorealistic rendering -->
    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/postprocessing/EffectComposer.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/postprocessing/RenderPass.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/postprocessing/ShaderPass.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/postprocessing/UnrealBloomPass.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/shaders/CopyShader.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/shaders/LuminosityHighPassShader.js">

    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/objects/Reflector.js">
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ambient-vignette {
            ...
            pointer-events: none;
            background:
                radial-gradient(ellipse 80% 60% at 50% -10%, rgba(56,189,248,0.07), transparent 60%),
                radial-gradient(ellipse 60% 50% at 100% 100%, rgba(168,85,247,0.06), transparent 60%),
        ...
        .btn-run-ctrl:hover { background: #2d3748; }
        .btn-run-ctrl.active-pause { background: rgba(245, 158, 11, 0.2); border-color: var(--accent-amber); color: var(--accent-amber); }
        ...
        .empty-state h2 { font-size: 17px; margin-bottom: 8px; color: var(--text-primary); }
        .empty-state p { font-size: 13px; }

        /* ===== CHART PANELS ===== */
        .chart-panel {
            background: linear-gradient(180deg, rgba(28, 33, 45, 0.96), rgba(22, 25, 34, 0.98));
            border: 1px solid var(--border);
            border-radius: 10px;
            overflow: hidden;
        ...
        .chart-panel:hover {
            ...
            box-shadow: 0 8px 28px rgba(0,0,0,0.45), 0 0 0 1px rgba(56,189,248,0.08), inset 0 1px 0 rgba(255,255,255,0.04);
        }
        ...
        .panel-header {
            ...
            padding: 10px 16px;
            border-bottom: 1px solid var(--border);
            background: linear-gradient(180deg, rgba(22,26,36,0.9), rgba(18,21,29,0.9));
        }
        ...
        .panel-close:hover { background: rgba(244,63,94,0.2); color: var(--accent-red); }
        ...
        .btn-action:hover { background: rgba(56, 189, 248, 0.25); }
        ...
        .btn-action.btn-green:hover { background: rgba(16, 185, 129, 0.25); }

        /* 3D Container & Overlays */
        .three-container {
            width: 100%;
            height: 100%;
            border-radius: 4px;
        ...
        .mission-desc strong { color: #f1f5f9; }

        .three-diagnostics-panel {
            position: absolute;
            top: 10px;
            left: 505px;
            z-index: 10;
        ...
        .diag-stat { display: flex; justify-content: space-between; align-items: center; }
        .diag-lbl { color: #94a3b8; }
        .diag-val { color: #f1f5f9; font-weight: 600; }
        .diag-val.text-green { color: #10b981; }
        .diag-val.text-amber { color: #f59e0b; }

        .three-controls-topright {
            position: absolute;
            top: 10px;
            right: 12px;
            display: flex;
        ...
        .filter-label { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: #94a3b8; margin-right: 3px; }
        ...
        .btn-filter:hover { color: #fff; background: #333d52; }
        .btn-filter.active { background: #38bdf8; color: #0f1117; border-color: #38bdf8; font-weight: 700; }

        .three-camera-presets { display: flex; gap: 4px; }
        ...
        .btn-cam:hover { background: #38bdf8; color: #0f1117; border-color: #38bdf8; font-weight: 600; }
        ...
        .det-card:hover {
            background: rgba(28, 33, 49, 0.95);
            border-color: rgba(56, 189, 248, 0.6);
            transform: translateY(-2px);
        }

        .det-name { font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 700; margin-bottom: 2px; }
        .det-function { font-size: 9.5px; color: #94a3b8; line-height: 1.3; }
        .det-function strong { color: #e2e8f0; }
        ...
        /* Fullscreen styles for 3D panel */
        ...
        .introjs-tooltiptext { color: #cbd5e1 !important; }
        .introjs-arrow.top { border-bottom-color: rgba(56, 189, 248, 0.4) !important; }
        .introjs-arrow.bottom { border-top-color: rgba(56, 189, 248, 0.4) !important; }
        .introjs-arrow.left { border-right-color: rgba(56, 189, 248, 0.4) !important; }
        .introjs-arrow.right { border-left-color: rgba(56, 189, 248, 0.4) !important; }
        .introjs-helperLayer { background: rgba(56, 189, 248, 0.08) !important; border: 1px solid rgba(56, 189, 248, 0.35) !important; }
        .introjs-button {
        ...
        .introjs-button:hover { background: #7dd3fc !important; }
        .introjs-skipbutton { color: #94a3b8 !important; }
        .introjs-tooltipbuttons { border-top: 1px solid rgba(255,255,255,0.08) !important; }
        ...
        .dqm-status-pill.good { background: rgba(16, 185, 129, 0.2); color: var(--accent-green); }
        .dqm-status-pill.warn { background: rgba(245, 158, 11, 0.2); color: var(--accent-amber); }
        .dqm-status-pill.bad { background: rgba(244, 63, 94, 0.2); color: var(--accent-red); }
        ...
        .dqm-sub { font-size: 10px; color: var(--text-secondary); }

        /* ================= ONLINE HISTOGRAM PRESENTER (OHP) STYLES ================= */
        .ohp-toolbar {
            display: flex;
            align-items: center;
            justify-content: space-between;
        ...
        .ohp-grid-btn:hover { color: #f1f5f9; background: rgba(255,255,255,0.06); }
        .ohp-grid-btn.active {
        ...
        .ohp-grid.cols-4 { grid-template-columns: repeat(4, minmax(0, 1fr)); }
        .ohp-grid.cols-2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        .ohp-grid.cols-1 { grid-template-columns: 1fr; }
        ...
        .ohp-expand-btn:hover {
            background: #0284c7;
            color: #ffffff;
            border-color: #0284c7;
        }

        /* OHP ENLARGED MODAL STYLES */
        .ohp-modal-overlay {
            position: fixed;
        ...
        .ohp-modal-overlay.active { display: flex; }
        .ohp-modal-container {
            background: #111520;
            border: 1px solid var(--border);
            border-radius: 10px;
        ...
        .ohp-modal-close:hover { color: #f43f5e; background: rgba(244,63,94,0.15); }
        .ohp-modal-body {
            padding: 16px;
            display: flex;
            gap: 16px;
        ...
        .root-stat-lbl { color: #94a3b8; }
        .root-stat-val { color: #f1f5f9; font-weight: 700; }
        ...
        .ohp-card canvas { display: block; width: 100%; height: 118px; }
    
        /* ================= DUAL-MODE 3D FULLSCREEN STYLES ================= */
        .three-container.is-fullscreen {
        ...
        .three-container.is-fullscreen canvas {
            width: 100vw !important;
            height: 100vh !important;
            display: block !important;
        }
        .three-container.is-fullscreen .three-exit-fullscreen-btn {
            display: flex !important;
        }
        .three-exit-fullscreen-btn {
            display: none;
            position: absolute;
            top: 14px;
            right: 14px;
        ...
        .three-exit-fullscreen-btn:hover {
            background: #f43f5e;
            transform: scale(1.05);
        }

        /* ================= BUILT-IN INTERACTIVE GUIDE MODAL ================= */
        ...
        .guide-card {
            ...
            flex-direction: column;
            gap: 16px;
            animation: guidePopIn 0.2s cubic-bezier(0.16, 1, 0.3, 1);
        }
        ...
        .guide-close:hover { color: #f43f5e; background: rgba(244, 63, 94, 0.1); }
        .guide-body {
            font-size: 13.5px;
            color: #cbd5e1;
            line-height: 1.6;
        }
        .guide-body strong { color: #38bdf8; }
        .guide-body kbd {
            background: #262c3d;
        ...
        .dvr-status-pill.live {
            background: rgba(244, 63, 94, 0.2);
            color: #f43f5e;
            border: 1px solid rgba(244, 63, 94, 0.4);
        }
        .dvr-status-pill.replay {
            background: rgba(56, 189, 248, 0.2);
            color: #38bdf8;
            border: 1px solid rgba(56, 189, 248, 0.4);
        }
        ...
        @keyframes pulseRec {
            from { opacity: 0.8; box-shadow: 0 0 4px #f43f5e; }
            to { opacity: 1; box-shadow: 0 0 12px #f43f5e; }
        }
        .dvr-timeline-container {
            flex: 1;
            display: flex;
            align-items: center;
            gap: 8px;
        ...
        /* ================= DAQ RUN CONTROL CONSOLE STYLES ================= */
        .tdaq-console-wrap {
            display: flex;
            flex-direction: column;
            height: 100%;
        ...
        .tdaq-state-node.completed {
            background: rgba(16, 185, 129, 0.12);
            color: #10b981;
            border-color: rgba(16, 185, 129, 0.35);
        }
        .tdaq-state-node.active-state {
            background: #10b981;
            color: #022c22;
            box-shadow: 0 0 14px rgba(16, 185, 129, 0.6);
            border-color: #34d399;
        }
        .tdaq-state-node.paused-state {
            background: #f59e0b;
            color: #451a03;
        ...
        .tdaq-header-val.green { color: #10b981; }
        .tdaq-header-val.amber { color: #f59e0b; }
        .tdaq-header-val.purple { color: #c084fc; }
        .tdaq-btn-group {
            display: flex;
            gap: 8px;
            flex-wrap: wrap;
        ...
        .tdaq-btn-cmd.btn-start {
            background: rgba(16, 185, 129, 0.2);
            border-color: #10b981;
            color: #10b981;
        }
        .tdaq-btn-cmd.btn-start:hover {
            background: #10b981;
            color: #042f2e;
        }
        .tdaq-btn-cmd.btn-pause {
            background: rgba(245, 158, 11, 0.2);
            border-color: #f59e0b;
            color: #f59e0b;
        }
        .tdaq-btn-cmd.btn-pause:hover {
            background: #f59e0b;
            color: #000;
        }
        .tdaq-btn-cmd.btn-stop {
            background: rgba(244, 63, 94, 0.2);
            border-color: #f43f5e;
            color: #f43f5e;
        }
        .tdaq-btn-cmd.btn-stop:hover {
            background: #f43f5e;
            color: #fff;
        }
        .tdaq-table {
        ...
        .tdaq-pill.ok { background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.4); }
        .tdaq-pill.warn { background: rgba(245, 158, 11, 0.2); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.4); }
        .tdaq-pill.err { background: rgba(244, 63, 94, 0.2); color: #f43f5e; border: 1px solid rgba(244, 63, 94, 0.4); }
        .tdaq-log-box {
            background: #090b10;
            border: 1px solid var(--border);
            border-radius: 6px;
        ...
        .tdaq-log-ts { color: #64748b; }
        .tdaq-log-src { color: #94a3b8; font-weight: 600; min-width: 140px; }
        .tdaq-log-lvl { font-weight: 700; border-radius: 3px; padding: 1px 6px; font-size: 9.5px; }
        .tdaq-log-lvl.INFO { color: #38bdf8; background: rgba(56,189,248,0.15); border: 1px solid rgba(56,189,248,0.3); }
        .tdaq-log-lvl.WARN { color: #f59e0b; background: rgba(245,158,11,0.15); border: 1px solid rgba(245,158,11,0.3); }
        .tdaq-log-lvl.ERROR { color: #f43f5e; background: rgba(244,63,94,0.15); border: 1px solid rgba(244,63,94,0.3); }

/* ================= COINCIDENCE WINDOW STYLES ================= */
        .coinc-summary-bar {
            display: flex;
            gap: 12px;
            background: #161a24;
        ...
        .coinc-stat-lbl { color: #94a3b8; font-size: 10px; }
        .coinc-stat-val { font-weight: 700; color: #10b981; font-size: 14px; }

    
        
        
        /* ================= MODERN HIGH-TECH CAEN HV CONTROL DASHBOARD ================= */
        .modern-hv-wrap {
    ...
    <!-- Intro.js for Interactive Tutorial -->
    ...
    <!-- App Configuration (Dynamic Overrides) -->
...
<!-- ===== AMBIENT CONTROL-ROOM BACKGROUND (animated drifting data particles + grid) ===== -->
...
<aside class="sidebar">
    <div class="sidebar-header">
        <h1 id="app-title">Event Explorer</h1>
        <div class="subtitle" id="app-subtitle">Modular Beamline DAQ</div>
    </div>
    ...
    <div class="tree-container">
        <div class="tree-root" id="treeRoot">
            ...
            <!--  Physics & 3D Reconstruction -->
            <div class="tree-category-title">3D & Physics Reconstruction</div>

            <div class="tree-sat">
                <div class="tree-sat-header expanded" onclick="toggleSat(this)">
                    <span>Event Displays & Invariant Mass</span>
                    <span class="chevron"></span>
                </div>
                <div class="tree-children open">
                    <div class="tree-view-item" onclick="openPanel('beamline_3d', 'Physics', '3D Interactive Beamline (π⁰ → γγ Dual Shower)', 'three_3d', true)" data-view="beamline_3d">
                        <span>3D Modular Beamline</span>
                        <span class="badge-tag highlight">Three.js</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('pi0_invariant_mass', 'Physics', 'Neutral Pion (π⁰) Invariant Mass Spectrum M(γγ)', 'pi0_mass')" data-view="pi0_invariant_mass">
                        <span>π⁰ Invariant Mass M(γγ)</span>
                        <span class="badge-tag highlight">M(π⁰)</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('pid_overview', 'Physics', 'Beam Composition (Proton / π⁰ / MIP PID)', 'pid_pie')" data-view="pid_overview">
                        <span>Beam Composition (PID)</span>
                        <span class="badge-tag">PID</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('pid_correlation', 'Physics', 'Calorimeter vs Cherenkov 2D Coincidence', 'pid_scatter')" data-view="pid_correlation">
                        <span>2D Coincidence Matrix</span>
                        <span class="badge-tag">2D</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('physics_fitting', 'Physics', 'Calorimeter Energy Peak Gaussian Fit', 'fit_hist')" data-view="physics_fitting">
                        <span>Gaussian Fit & Resolution</span>
                        <span class="badge-tag">Fit</span>
                    </div>
                    

                    
                    <div class="tree-view-item" onclick="openPanel('geco_panel', 'Slow Control', 'CAEN GECO2020 High Voltage', 'geco_panel', true)" data-view="geco_panel">
                    ...
                    <div class="tree-view-item" onclick="openPanel('dfp_panel', 'TDAQ System', 'DFP Panel & Network', 'dfp_panel', true)" data-view="dfp_panel">
                        <span>DFP Panel</span>
...
<div class="tree-view-item" onclick="openPanel('coincidence_view', 'TDAQ System', 'Coincidence Timing Window & Noise Rejection', 'coincidence_view', true)" data-view="coincidence_view">
                        <span>Coincidence Window (±10ns)</span>
                        <span class="badge-tag" style="background:rgba(16,185,129,0.2);color:#10b981;border:1px solid #10b981;">TDAQ</span>
                    </div>

                    <div class="tree-view-item" onclick="openPanel('ohp_grid', 'TDAQ System', 'Online Histogram Presenter — Scaler Grid', 'ohp_view', true)" data-view="ohp_grid">
                        <span>Online Histogram Presenter</span>
                        <span class="badge-tag highlight" style="background:rgba(56,189,248,0.15);color:#38bdf8;border:1px solid #38bdf8;">OHP</span>
                    </div>

                    <div class="tree-view-item" onclick="openPanel('dqm_summary', 'System', 'Data Quality Monitoring (DQM) & Export', 'dqm_view', true)" data-view="dqm_summary">
                        <span>DQM Health & Export</span>
                        <span class="badge-tag">DQM</span>
                    </div>
                </div>
            </div>

            <!-- 🛰️ Detector Satellites -->
            <div class="tree-category-title">Detector Satellites</div>
            ...
            <div class="tree-sat">
                ...
                <div class="tree-children">
                    <div class="tree-view-item" onclick="openPanel('calorimeter_energy', 'Calorimeter', 'Energy Histogram (16 ch ADC)', 'bar')" data-view="calorimeter_energy">
                        <span>Energy Histogram (16 ch)</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('calorimeter_heatmap', 'Calorimeter', 'Channel Heatmap 4×4 Matrix', 'heatmap')" data-view="calorimeter_heatmap">
                        <span>Channel Heatmap (4×4)</span>
                    </div>
                </div>
            </div>
            ...
            <div class="tree-sat">
                ...
                <div class="tree-children">
                    <div class="tree-view-item" onclick="openPanel('scintillator_timing', 'Scintillator', 'TDC Timing Distribution (ns)', 'histogram')" data-view="scintillator_timing">
                        <span>Timing Distribution</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('scintillator_pe', 'Scintillator', 'Photoelectron Multiplicity', 'histogram')" data-view="scintillator_pe">
                        <span>Photoelectron Count</span>
                    </div>
                </div>
            </div>
            ...
            <div class="tree-sat">
                <div class="tree-sat-header" onclick="toggleSat(this)" data-sat="DWC">
                    <span>DWCSatellite</span>
                    <span class="chevron"></span>
                </div>
                <div class="tree-children">
                    <div class="tree-view-item" onclick="openPanel('dwc_hitmap', 'DWC', 'DWC Beam Profile (100×100mm)', 'heatmap2d_dwc')" data-view="dwc_hitmap">
                        <span>DWC Hit Map</span>
                        <span class="badge-tag">DWC</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('dwc_delta_t', 'DWC', 'DWC Wire Delay Δt (ns)', 'histogram')" data-view="dwc_delta_t">
                        <span>Wire Delay Δt</span>
                    </div>
                </div>
            </div>
            ...
            <div class="tree-sat">
                ...
                <div class="tree-children">
                    <div class="tree-view-item" onclick="openPanel('timepix_hitmap', 'Timepix', 'Pixel Hit Matrix (256×256)', 'heatmap2d')" data-view="timepix_hitmap">
                        <span>Hit Map (256×256)</span>
                    </div>
                    <div class="tree-view-item" onclick="openPanel('timepix_tot', 'Timepix', 'Time-over-Threshold (ToT) Spectrum', 'histogram')" data-view="timepix_tot">
                        <span>ToT Spectrum</span>
                    </div>
                </div>
            </div>
            ...
            <div class="tree-sat">
                ...
                <div class="tree-children">
                    <div class="tree-view-item" onclick="openPanel('cherenkov_qdc', 'Cherenkov', 'Cherenkov QDC Spectrum', 'histogram')" data-view="cherenkov_qdc">
                        <span>QDC Spectrum (PID)</span>
                    </div>
                </div>
            </div>
...
<main class="main-content">
    ...
    <!-- ===== DVR PLAYBACK & RECORDING BAR ===== -->
    <div class="dvr-bar" id="dvrBar">
        <div class="dvr-controls">
            <span class="dvr-status-pill live" id="dvrStatusPill">● LIVE STREAM</span>
            <button class="btn-dvr" id="btnDvrRec" onclick="toggleDvrRecord()">
                <span id="recDot">🔴</span> <span id="recText">Record Clip</span>
            </button>
            <label class="btn-dvr" style="margin:0; cursor:pointer;">
                <span>📂</span> <span>Load Run</span>
                <input type="file" id="dvrFileInput" accept=".json" style="display:none;" onchange="loadDvrFile(event)">
            </label>
            <button class="btn-dvr" onclick="exportDvrClip()">
                <span>💾</span> <span>Save Clip</span>
            </button>
        </div>

        <div class="dvr-timeline-container">
            <button class="btn-dvr" onclick="stepDvr(-10)">⏮ -10</button>
            <button class="btn-dvr" onclick="stepDvr(-1)"> Step</button>
            <button class="btn-dvr" id="btnDvrPlay" onclick="toggleDvrPlay()">Play</button>
            <button class="btn-dvr" onclick="stepDvr(1)">Step </button>
            <button class="btn-dvr" onclick="stepDvr(10)">+10 ⏭</button>
            <input type="range" class="dvr-slider" id="dvrSlider" min="0" max="0" value="0" oninput="seekDvr(this.value)">
            <span id="dvrFrameText" style="min-width: 80px; text-align:right;">0 / 0</span>
        </div>

        <div class="dvr-controls">
            <span>Speed:</span>
            <select class="dvr-speed-select" id="dvrSpeedSelect" onchange="setDvrSpeed(this.value)">
                <option value="0.5">0.5x</option>
                <option value="1.0" selected>1.0x</option>
                <option value="2.0">2.0x</option>
                <option value="5.0">5.0x</option>
            </select>
            <button class="btn-dvr" onclick="returnToLiveStream()" style="border-color:#10b981; color:#10b981;">
                <span></span> <span>Jump to Live</span>
            </button>
        </div>
    </div>
    ...
    <div class="top-bar">
        ...
        <div class="controls-group">
            ...
            <button class="btn-run-ctrl" id="btnPauseResume" onclick="toggleStreamPause()">
                <span id="pauseIcon"></span> <span id="pauseText">Pause Stream</span>
            </button>
            <button class="btn-run-ctrl" onclick="snapshotAllBuffers()">
                <span></span> <span>Take Snapshot</span>
            </button>
        </div>
        <div class="stats">
            <div class="stat-item" style="display:flex; align-items:center; gap:6px;">
                <span>Beamline:</span>
                <span class="stat-value" id="beamStatusBadge" style="padding:3px 10px; border-radius:12px; font-weight:700; font-size:11px; background:rgba(30,41,59,0.8); color:#94a3b8; border:1px solid #334155; transition: all 0.3s ease;">○ BEAM STANDBY</span>
            </div>
            <div class="stat-item">
                <span>Instant Rate:</span>
                <span class="stat-value" id="eventsPerSec">0 ev/s</span>
            </div>
            <div class="stat-item">
            ...
            <div class="stat-item">
                <span>π⁰ Fraction:</span>
                <span class="stat-value" id="topPidRatio">--</span>
            </div>
            <div class="stat-item">
                <span>Panels:</span>
    ...
    <div class="panels-container" id="panelsContainer">
        <div class="empty-state" id="emptyState">
            ...
            <p>Select a detector or physics analysis panel from the sidebar to begin live monitoring.</p>
        </div>
    </div>
</main>
...
function openPanel(viewId, satellite, title, chartType, isFullWidth = false) {
    ...
    if (chartType === 'heatmap2d' || chartType === 'heatmap2d_dwc') {
        ...
        bodyContent = `<div id="ohp-container" class="ohp-container"></div>`;
        footerButtons = `
...
let tdaqRateHistory = Array(30).fill(0);
let tdaqTimeHistory = Array(30).fill('');

function createTdaqRateChart(chart) {
    chart.setOption({
        backgroundColor: 'transparent',
        title: {
            text: '⚡ L1 Trigger & Particle Instantaneous Extraction Rate (CERN T9)',
            textStyle: { color: '#38bdf8', fontFamily: 'JetBrains Mono', fontSize: 11, fontWeight: 'bold' },
            left: 8, top: 4
        },
        tooltip: {
            trigger: 'axis',
            ...
            formatter: (params) => {
                let p = params[0];
                let isSpill = Number(p.value) > 20;
                let status = isSpill ? '<span style="color:#10b981;font-weight:bold;">● BEAM ON (Spill Active)</span>' : '<span style="color:#f59e0b;">○ BEAM OFF (Inter-Spill: 9.6s)</span>';
                return `Time: <strong>${p.name}</strong><br/>Rate: <strong>${p.value} Hz</strong><br/>State: ${status}`;
            }
        },
        grid: { top: 32, right: 15, bottom: 22, left: 48 },
        xAxis: {
            type: 'category',
            data: tdaqTimeHistory,
        ...
        series: [{
            ...
            markLine: {
                ...
                data: [{ yAxis: 400, label: { formatter: 'Target Extraction (400 Hz)', color: '#10b981', fontFamily: 'JetBrains Mono', fontSize: 9 } }]
            }
        }]
    });
}
...
function updateAllPanels() {
    ...
    // Top Bar & Beamline Spill State Update
    const now = Date.now();
    if (now - lastSecondTimestamp >= 1000) {
        const rate = eventCountThisSecond;
        document.getElementById('eventsPerSec').textContent = `${rate.toLocaleString()} ev/s`;
        
        // Update Beamline extraction badge
        const beamBadge = document.getElementById('beamStatusBadge');
        if (beamBadge) {
            if (rate > 15) {
                beamBadge.innerHTML = '● BEAM ON <span style="font-size:9px;opacity:0.85;">(400ms Spill)</span>';
                beamBadge.style.background = 'rgba(16, 185, 129, 0.2)';
                beamBadge.style.color = '#10b981';
                beamBadge.style.borderColor = '#10b981';
                beamBadge.style.boxShadow = '0 0 12px rgba(16, 185, 129, 0.45)';
            } else {
                beamBadge.innerHTML = '○ INTER-SPILL <span style="font-size:9px;opacity:0.85;">(9.6s Gap)</span>';
                beamBadge.style.background = 'rgba(245, 158, 11, 0.12)';
                beamBadge.style.color = '#f59e0b';
                beamBadge.style.borderColor = 'rgba(245, 158, 11, 0.4)';
                beamBadge.style.boxShadow = 'none';
            }
        }
        
        // Push to TDAQ Rate waveform history
        tdaqRateHistory.push(rate);
        tdaqRateHistory.shift();
        const timeStr = new Date().toLocaleTimeString().split(' ')[0];
        tdaqTimeHistory.push(timeStr);
        tdaqTimeHistory.shift();
        
        // Update active TDAQ Run Control charts if open
        for (const [vId, p] of Object.entries(activePanels)) {
            if (p.chartType === 'run_control_console' && p.chart && p.chart.chart) {
                p.chart.chart.setOption({
                    xAxis: { data: tdaqTimeHistory },
                    series: [{ data: tdaqRateHistory }]
                });
                const rateValEl = document.getElementById('tdaq-val-rate');
                if (rateValEl) rateValEl.textContent = `${rate} Hz`;
            }
        }
...
function renderOhpGrid() {
        ...
        for (const key of Object.keys(OHP_CHANNEL_LABELS)) {
            html += `
                <div class="ohp-card">
                    ...
                    <div id="ohp-canvas-${key}" style="flex:1; width:100%; min-height:140px;"></div>
                </div>
            `;
        }
        html += `</div>`;
    ...
    for (const key of Object.keys(OHP_CHANNEL_LABELS)) {
        drawOhpMiniPlot(`ohp-canvas-${key}`, dataBuffers.ohp_series[key] || []);
    }
...
function updateOhpModalPlot() {
    ...
    drawOhpMiniPlot('ohp-modal-canvas', series);
}

const ohpEchartsInstances = {};

function drawOhpMiniPlot(domId, series) {
    const dom = document.getElementById(domId);
    if (!dom) return;

    if (!ohpEchartsInstances[domId]) {
        ohpEchartsInstances[domId] = echarts.init(dom);
    }
    const chart = ohpEchartsInstances[domId];
    ...
    const option = {
        ...
        tooltip: { trigger: 'axis', backgroundColor: 'rgba(255,255,255,0.9)', borderColor: '#999', textStyle: { color: '#000' } },
        dataZoom: [
            { type: 'inside', xAxisIndex: 0, filterMode: 'none' },
            { type: 'inside', yAxisIndex: 0, filterMode: 'none' }
        ],
        grid: { left: '40px', right: '15px', top: '15px', bottom: '20px' },
        xAxis: { 
            type: 'category', 
        ...
        graphic: [
            {
                type: 'group', right: 5, top: 5,
                children: [
                    { type: 'rect', z: 100, shape: { width: 75, height: 42 }, style: { fill: 'rgba(255,255,255,0.92)', stroke: '#94a3b8' } },
                    { type: 'text', z: 100, left: 4, top: 4, style: { fill: '#334155', text: `Entries ${entries.toFixed(0)}\nMean    ${mean.toFixed(1)}\nStd Dev ${stdDev.toFixed(1)}`, font: '9px "JetBrains Mono", monospace' } }
                ]
            }
        ],
        series: [
            {
                ...
                lineStyle: { width: 1.5, type: 'step' },
                showSymbol: false,
            },
            {
                type: 'scatter',
                data: values.length ? [[values.length - 1, values[values.length - 1]]] : [],
...
<!-- ================= BUILT-IN INTERACTIVE GUIDE MODAL ================= -->
<div id="interactive-guide-modal">
    <div class="guide-card">
        <div class="guide-header">
            <div class="guide-title">
                <span id="guide-icon"></span>
                <span id="guide-title-text">Welcome to BL4S 3D Event Explorer</span>
            </div>
            <div style="display: flex; align-items: center; gap: 8px;">
                <span class="guide-step-badge" id="guide-step-badge">Step 1 of 7</span>
                <button class="guide-close" onclick="close3DGuide()">✕</button>
            </div>
        </div>
        <div class="guide-body" id="guide-body-text">
            Loading guide...
        </div>
        <div class="guide-footer">
            <div class="guide-progress-dots" id="guide-dots">
                <!-- Dots generated by JS -->
            </div>
            <div class="guide-btn-group">
                <button class="btn-guide" id="guide-btn-prev" onclick="prev3DGuideStep()"> Back</button>
                <button class="btn-guide primary" id="guide-btn-next" onclick="next3DGuideStep()">Next </button>
            </div>
        </div>
    </div>
</div>
...
<!-- OHP ENLARGED HISTOGRAM MODAL -->
<div class="ohp-modal-overlay" id="ohp-modal-overlay">
    <div class="ohp-modal-container">
        <div class="ohp-modal-header">
            <div class="ohp-modal-title">
                <span></span>
                <span id="ohp-modal-title-text">Histogram Explorer</span>
            </div>
            <button class="ohp-modal-close" onclick="closeOhpModal()">✕</button>
        </div>
        <div class="ohp-modal-body">
            <div class="ohp-canvas-wrap">
                <div id="ohp-modal-canvas" style="width:100%; height:380px; display:block;"></div>
            </div>
            <div class="ohp-modal-stats">
                <div class="root-stat-header">ROOT Statistics</div>
                <div class="root-stat-row"><span class="root-stat-lbl">Entries:</span><span class="root-stat-val" id="ohp-stat-entries">0</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Mean:</span><span class="root-stat-val" id="ohp-stat-mean">0.00</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Std Dev:</span><span class="root-stat-val" id="ohp-stat-std">0.00</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">RMS:</span><span class="root-stat-val" id="ohp-stat-rms">0.00</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Integral:</span><span class="root-stat-val" id="ohp-stat-integral">0</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Peak Val:</span><span class="root-stat-val" id="ohp-stat-peak">0</span></div>
                
                <div class="root-stat-header" style="margin-top:12px;">Histogram Info</div>
                <div class="root-stat-row"><span class="root-stat-lbl">Type:</span><span class="root-stat-val" style="color:#10b981;">TH1D Scaler</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Sampling:</span><span class="root-stat-val">500 ms</span></div>
                <div class="root-stat-row"><span class="root-stat-lbl">Window:</span><span class="root-stat-val">Rolling 60s</span></div>
            </div>
        </div>
    </div>
</div>
